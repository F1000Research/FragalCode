...
my ($justone,$table,$save,$findmatch,@expressions);
my $simi ;
my $verbose = 0 ;
my $verb;
...
my $outfileforneedle = "/tmp/needle.out";
unlink $outfileforneedle ;
...
while(@files){
    ...
    foreach my $f2 (@files){
	   ...
	   #$repeat->{$f2} = 1 ;
	   print "Writing file $outfileforneedle\n" if($verbose);

	   if(! -e $outfileforneedle){
	       BP_Needle($outfileforneedle,"/",$f1,$f2); 
	   }
	   my ($iden,$simival) = BP_Needle_parseout($outfileforneedle);
	   ...
	   util_printAndDo("cp -f $outfileforneedle .") if(defined $save);
	   unlink $outfileforneedle ;
    }
