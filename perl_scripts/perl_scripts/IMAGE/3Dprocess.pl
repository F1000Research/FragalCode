...
usage( "Need to give a specifiedcolor ile name => option -infile ") if(!defined $specifiedcolor );
...
foreach my $i (@list){
    ...
    my (@red) = $image->FindCoordsForColor($specifiedcolor);
	my $nblue = @blue ;
	my $nred = @red ;
