...
while(@threshes){
	...
	$link1 = web_makeLink("Sorted based on average for %similarity+identity, even if not annotated",$s1);
	push @links, $link1 ;

	$s1 = "aver.results.thresh$thresh.size$size.doiden1.onlyanno1.html";
	$link1 = web_makeLink("Sorted based on average for %similarity+identity, if annotated",$s1);
	push @links, $link1 ;
	...
	$link1 = web_makeLink("Sorted based on cumulative score for %similarity+identity, even if not annotated",$s1);
	push @links, $link1 ;


	$s1 = "cumu.results.thresh$thresh.size$size.doiden1.onlyanno1.html";
	$link1 = web_makeLink("Sorted based on cumulative score for %similarity+identity, if annotated",$s1);
	push @links, $link1 ;
