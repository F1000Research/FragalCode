...
echo "Making rr $ref"
      echo \#\!/bin/csh \-f > ! /tmp/sandeep.1
      echo $SRC/3DMatch -annd $anndir -outf $dir/$ref/$ref.pdb.out -pdb $ref -inc $anndir/$ref.outconf.annotated -fin -close2activesite $close2activesite >> /tmp/sandeep.1
      #bsub < /tmp/sandeep  > & /dev/null 
	  source /tmp/sandeep.1
  endif 
  ...
  foreach query ( ` cat $listquery` )
       ...
       if( ! -e $query.pdb.out   ) then 
            echo $ref $query 
           echo \#\!/bin/csh \-f > ! /tmp/sandeep.1
           echo $SRC/3DMatch -annd $anndir -outf $dir/$ref/$query.pdb.out -pdb $query -inc $anndir/$ref.outconf.annotated -fin -close2activesite $close2activesite >> /tmp/sandeep.1
           #bsub < /tmp/sandeep  > & /dev/null 
	       source /tmp/sandeep.1
  endif 
